...
#ifndef ADS7039_INRANGEDETECTOR_H_
#define ADS7039_INRANGEDETECTOR_H_
...
public:
	...
	void init(const T min, const T max, const uint32_t dt) { m_minimum = min; m_maximum = max; m_dt = dt; m_ms_counter = 0; m_lastError = error::None; }
	void reset();
private:
	uint32_t m_ms_counter = 0;
...
typename InRangeDetector<T>::error InRangeDetector<T>::proceed(const uint32_t current_time, const T value)
{
	...
	if(reg_error != error::None) {
		const uint32_t next_counter = m_ms_counter + 1;

		if(next_counter < m_dt) {
			m_ms_counter = next_counter;
		} else if(next_counter == m_dt) {
			m_lastError = reg_error;
		}
	} else {
		const int next_counter = m_ms_counter - 1;
...
#endif /* ADS7039_INRANGEDETECTOR_H_ */
